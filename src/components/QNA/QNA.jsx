...
export const QNA = ({ quizData }) => {
  ...
  useEffect(() => {
    if (quiz.length > 0) {
      ...
      setLoading(false); // handle empty quiz
    }
  }, [quiz]);
  ...
  if (!quiz || quiz.length === 0) return <p className="text-center">No quiz questions available.</p>;
    ...
    <main className="d-flex justify-center qns-main">
      <section className="question-dialog container-flex">
        ...
        <div className="question">
          <span>Q{index + 1}: {currentQuestion?.question}</span>
        </div>
